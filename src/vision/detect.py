...
def draw(frame, detections, enrichments: dict | None = None):
    for d in detections:
        x1, y1, x2, y2 = map(int, d.bbox)
        color = CLASS_COLORS.get(d.class_name, (255, 255, 255))
        cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
        label = f"{d.class_name} {d.confidence:.2f}"
        if enrichments and d.class_name in enrichments:
            e = enrichments[d.class_name]
            label += f" | {e.action} [{e.bin_color}]"
...
def main():
    ...
    parser.add_argument("--rag", action="store_true",
                        help="Enable RAG enrichment (requires built index)")
    ...
    print(f"Running CleanBot detector | weights={args.weights} conf={args.conf} rag={args.rag}")
    try:
        while True:
            ok, frame = cap.read()
            if not ok:
            ...
            for d in detections:
                if rag:
                    e = rag.enrich(d.class_name, d.confidence)
                    enrichments[d.class_name] = e
                    print(
                        f"  {d.class_name:10s} conf={d.confidence:.2f} "
                        f"action={e.action} bin={e.bin_color} "
                        f"priority={e.priority} hazard={e.hazard}"
                    )
                else:
                    print(f"  {d.class_name:10s} conf={d.confidence:.2f} bbox={tuple(map(int, d.bbox))}")

            if args.show:
                cv2.imshow("CleanBot", draw(frame, detections, enrichments or None))
